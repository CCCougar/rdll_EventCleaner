...
HANDLE fn_trav_proc_handle(DWORD dwProcessId) {
	...
	for (INT i = 0; i <= psHandleInfo->HandleCount; i++) {
		...
		posi = stl_str.find("\\application.evtx");
		if (posi != stl_str.npos) {
			include = TRUE;
		}
		posi = stl_str.find("\\system.evtx");
		if (posi != stl_str.npos) {
			include = TRUE;
		}
		...
		if (include) {
			...
			if (bRet == 0)
				...
				cout << "[+] Closed handle: 0x" << std::hex << psHandle.Handle << " in processID: " << std::dec << psHandle.ProcessId << endl;
		}
